...
TLS_VAR int optind = 1;

/* Formerly, initialization of getopt depended on optind==0, which
   causes problems with re-calling getopt as programs generally don't
   know that. */

TLS_VAR int __getopt_initialized;
...
TLS_VAR static char *nextchar;

/* Callers store zero here to inhibit the error message
   for unrecognized options.  */

TLS_VAR int opterr = 1;
...
TLS_VAR static char *posixly_correct;
...
TLS_VAR static int first_nonopt;
TLS_VAR static int last_nonopt;

#ifdef _LIBC
/* Stored original parameters.
   XXX This is no good solution.  We should rather copy the args so
   that we can compare them later.  But we must not use malloc(3).  */
TLS_VAR extern int __libc_argc;
TLS_VAR extern char **__libc_argv;
...
TLS_VAR static int nonoption_flags_max_len;
TLS_VAR static int nonoption_flags_len;
# endif
